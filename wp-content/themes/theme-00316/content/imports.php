...
//Theme_Content_Import
	...
	{	
		...
		$post_mail = array(
			'subject' => sprintf(
				_x( '%1$s "%2$s"', 'mail subject', 'theme--00316' ),
				get_bloginfo( 'name' ), '['.$mail_arr[3].']' ),
			'sender' => sprintf( '['.$mail_arr[1].'] <%s>', '['.$mail_arr[2].']' ),
			'body' =>
				sprintf( __( 'From: %s', 'theme--00316' ),
					'['.$mail_arr[1].'] <['.$mail_arr[2].']>' ) . "\n"
				. sprintf( __( 'Subject: %s', 'theme--00316' ),
					'['.$mail_arr[3].']' ) . "\n\n"
				. __( 'Message Body:', 'theme--00316' )
					. "\n" . '[your-message]' . "\n\n"
				. '-- ' . "\n"
				. sprintf( __( 'This e-mail was sent from a contact form on %1$s (%2$s)',
					'theme--00316' ), get_bloginfo( 'name' ), home_url() ),
